...
## 5. Scenario Sizing (Illustrative Uplift)

### 5.1 Reduce At-Risk Churn by 10%

- **Inputs**:
  - N_{AR}: number of At-Risk customers.  
  - Churn_{AR}: annual churn rate in At-Risk.  
  - ARPU_{AR}: average annual revenue per At-Risk customer.
- **Logic**:

\text{Saved customers} = 0.1 \times Churn_{AR} \times N_{AR}

\text{Annual uplift} \approx \text{Saved customers} \times ARPU_{AR}

- **Interpretation**: even modest churn reduction in this high-value group produces material retained revenue.

---

### 5.2 Convert 5% of Need Attention into Loyal

- **Inputs**:
  - N_{NA}: number of Need Attention customers.  
  - ARPU_{NA}: annual revenue per Need Attention customer.  
  - ARPU_{L}: annual revenue per Loyal customer.
- **Logic**:

\text{Converted customers} = 0.05 \times N_{NA}

\Delta ARPU = ARPU_{L} - ARPU_{NA}

\text{Annual uplift} \approx \text{Converted customers} \times \Delta ARPU

- **Interpretation**: small conversion rates applied to a large low-value base can generate substantial incremental revenue.

---

## 6. KPIs and Monitoring Framework

### 6.1 Core Segment-Level KPIs
